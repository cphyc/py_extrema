...
def testGradientHessian():
    ...
    assert np.allclose(grad[1:-1], fd_grad[1:-1],
                       atol=np.abs(fd_grad).max()/100)
    assert np.allclose(hess[2:-2], fd_hess[2:-2],
                       atol=np.abs(fd_hess).max()/100)
...
def testSmoothing1D():
    ...
    data_exact = 1/np.sqrt(2*np.pi*sigma**2)*np.exp(-(x-N//2)**2/(2*sigma**2))
    ...
    if _PLOT:
        ...
        plt.plot(1/np.sqrt(2*np.pi*sigma**2)*np.exp(-(x-N//2)**2/(2*sigma**2)),
                 ls='--',
                 label='Exact')
        plt.legend()
